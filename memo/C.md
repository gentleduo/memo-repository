...
6. -i( -include：i 的小写) ：包含头文件名字，用来包含头文件名字，但一般情况下包含头文件都在源码里用#include xxxxxx实现，-include参数很少用 (一般不使用，而是直接放在.c 文件中通过#include<***.h> 添加)；
7. -I (include_path的i大写) ：头文件路径
   1. 指定头文件路径（相对路径或觉得路径，建议相对路径），/usr/include目录一般是不用指定的，gcc知道去那里找；
   2. 如果头文件不在/usr/include里就要用-I参数指定了，比如头文件放在/myinclude目录里，那编译命令行就要加上-I/myinclude参数了，如果不加你会得到一个"xxxx.h: No such file or directory"的错误。`
   3. -I参数可以用相对路径，比如头文件在当前目录可以用-I.来指定；
8. -I  library，链接指定的库名。
9. -L  dir，添加库文件搜索路径。

>**-l参数和-L参数**
>
>-l参数 就是用来指定程序要链接的库的库名。比如数学库：它的库文件名是libm.so，它的库名就是m，很容易看出，把库文件名的头lib和尾.so去掉就是库名了。再比如：如果要用到一个第三方提供的库名字叫libtest.so，那么只要把libtest.so拷贝到/usr/lib里，编译时加上-ltest参数，就能用上libtest.so库了。
>
...
3. 再找内定目录 /lib 、 /usr/lib 、 /usr/local/lib这是当初compile gcc时写在程序内的 （根据操作系统版本可能有所不同：/lib64）
...
4. 默认的动态库搜索路径/lib（根据操作系统版本可能有所不同：/lib64）
5. 默认的动态库搜索路径/usr/lib（根据操作系统版本可能有所不同）
...
使用“:=”定义的变量称为“简单展开变量（simply expanded variables）”，顾名思义，“:=”右边的表达式如果包含对变量的引用，则这些对变量的引用会直接展开，得到一个确切的值，并赋给“:=”左边的变量。即：会在变量的定义点，按照被引用的变量的当前值进行展开，这种定义变量的方式更适合在大的编程项目中使用，因为它更像一般的编程语言，例如：
...
| AS   | 汇编器的名字，缺省值是as。（用汇编器从汇编代码生成目标代码） |
| LD   | 链接器的名字，缺省值是ld。                                   |
| CC   | C编译器的名称，默认值为cc(cc和gcc都是c的编译命令，都能编译c代码)。CPP  C预编译器的名称，默认值为$(CC) –E。 |
| CXX  | C++编译器的名称，默认值为g++。                               |
...
用动态存储分配函数动态开辟的空间，在使用完毕后未释放，结果导致一直占据该内存单元。直到程序结束。即所谓内存泄漏。（注意：内存泄漏是指堆内存的泄漏。）简单的说就是申请了一块内存空间，使用完毕后没有释放掉。它的一般表现方式是程序运行时间越长，占用内存越多，最终用尽全部内存，整个系统崩溃。由程序申请的一块内存，且没有任何一个指针指向它，那么这块内存就泄露了。
...
不是NULL指针，是指向“垃圾”内存(指向预期之外的内存)的指针。一般不会错用NULL指针，因为用if语句很容易判断。但是“野指针”是很危险的，if语句对它不起作用。“野指针”是很危险的。“野指针”的成因：

1. 指针变量没有被初始化。任何指针变量刚被创建时不会自动成为NULL指针，它的缺省值是随机的，它会乱指一气。所以，指针变量在创建的同时应当被初始化，要么将指针设置为NULL，要么让它指向合法的内存。
2. 指针p被free之后，没有置为NULL，让人误以为p是个合法的指针。free和delete只是把指针所指的内存给释放掉，但并没有把指针本身干掉。通常会用语句if (p != NULL)进行防错处理。很遗憾，此时if语句起不到防错作用，因为即便p不是NULL指针，它也不指向合法的内存块。
...
int main()
{
        ...
        //p=NULL;
        if(p != NULL) {
                strcpy(p, "world"); // 这里可以运行并且，并且也成功打印了，但是很危险，因为p指向的内存空间很有可能又被分配给其他指针了
                printf("置NULL之后：%s \n",p);
        }
        p=NULL;
}
...
int main() {
        ...
        printf("%%d表示十进制整数:a=%d;\n",a);
        int b = 0xA;
        printf("%%x表示十六进制无符号整数,#表示在八进制和十六机制数前显示前导0和0x:b=%#x;\n",b);
        int c = 07;
        printf("%%o表示八进制无符号整数,#表示在八进制和十六机制数前显示前导0和0x:c=%#o;\n",c);
        unsigned int d = 1234;
        printf("%%u表示无符号十进制整数:d=%u;\n",d);
        char e = 'a';
        printf("%%c表示单一字符:e=%c;\n",e);
        char f[] = "Hello,world!";
        printf("%%s表示字符串:f=%s;\n",f);
        float g = 10000;
        printf("%%e表示指数形式浮点小数:g=%e;\n",g);
        printf("%%f表示小数形式浮点小数:g=%f;\n",g);
        printf("%%p表示地址:p=%p;\n",f);
        char h[] = "Hello";
        printf("%%m.ns对于字符串m表示输出数据域宽,n表示实际输出位数,因此最终输出的长度以n为准,当n大于m时输出字符串的前n位,当n小于m时输>出字符串的前n位的同时前面补空格:h=%3.6s\n",h);
        float i = 1234.5678;
        printf("%%m.nf对于实数m表示输出数据域宽,n指定小数点后位数(四舍五入),只有当m的值大于四舍五入之后的实数的位数的时候才进行补零或者补空格,如果m的值小于四舍五入之后的实数的位数则输出原实数按照n位进行四舍五入后的结果:i=%2.2f\n",i);
        int j = 1234;
        printf("%%-8d表示输出数据在域内左对齐(缺省右对齐):j=%-8d,j=%8d;\n",j,j);
        int k = 1234;
        printf("%%+d指定在有符号数的正数前显示正号(+):k=%+d;\n",k);
        int l = 1234;
        printf("%%8d输出数值时指定左边不使用的空位置自动填0:l=%08d;\n",l);
}
```
...
4. 数组名表示内存首地址，是地址常量；sizof(数组名)是数组占用的总内存空间
...
int main() {
        ...
        //int a[5],i; // 数组中的元素被赋随机数;
        //int a[5]={1,2,3},i; // 等价于：a[0]=1; a[1]=2;a[2]=3; a[3]=0; a[4]=0;
...
int main() {
        ...
        // 因为数组的指针是一个常量(即：地址)，所以不能进行赋值，比如下面的代码编译会报错：
        // int a[5]={1,2,3,4,5};
        // int b[5];
        // b = a;
    	return 0;
...
>
> 数组名是符号常量，代表数组首地址，自身无存储单元，也就是说不存在一个有意义的地址来保存这个常量，或者说在内存中你无法找到存储这个常量自身的地址，因此这个值是不能修改的。
>
> 常量其实分为符号常量和值常量比如你要计算s=1+1，“=”、“+”都不分配内存单元，为什么？因为他们是符号常量，或者叫运算符，这些东西只在你编写程序的时候出现，编译之后，这些东西都会转化为机器指令，并没有地址去存储这些东西，数组名应该是和它相类似。至于里面的数字1会不会分配地址需要看具体编译软件不同指令集，不同操作系统会有差异，但一般来讲至少会有一个1有存储单元，到底有没有需要编译之后对EXE文件进行反汇编。
...
在二维数组中a＋1和*(a＋1)的值虽然相等，但是类型不相等，a+1是int **类型，指向的是数组a的第二行的起始地址; *(a+1)是int *型，指向的是数组a的第二行第一个元素的地址。
...
2. 包括数据的逻辑结构和存储结构及其操作;
   ...
   - 逻辑结构：表示数据运算之间的抽象关系，按每个元素可能具有的直接前趋数和直接后继数将逻辑结构分为“线性结构”和“非线性结构”两大类。
     ...
     - 线性结构：线性结构指的是数据元素之间存在着“一对一”的线性关系的数据结构，如（a0,a1,a2,.....,an）,a0为第一个元素，an为最后一个元素，此集合即为一个线性结构的集合；相对应于线性结构，非线性结构的逻辑特征是一个结点元素可能对应多个直接前驱和多个后继：
       - 集合中必存在唯一的一个"第一个元素"；
       - 集合中必存在唯一的一个"最后的元素"；
       - 除最后元素之外，其它数据元素均有唯一的"后继"；
       - 除第一元素之外，其它数据元素均有唯一的"前驱"。
     - 树形结构：树状结构是一个或多个节点的有限集合。
     - 图状结构：多个对多个，如图
