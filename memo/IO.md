...
1. 为了防止历史连接中的数据，被后面相同四元组的连接错误的接收。（唯一连接确认方式为四元组：源IP地址、目的IP地址、源端口、目的端口），这个时间足以让两个方向上的数据包都被丢弃，使得原来连接的数据包在网络中都自然消失，再出现的数据包一定都是新建立连接所产生的。
...
2. 另外一个作用是，防止远程因为最后一个ACK包丢失，导致停留在LAST-ACK状态，从而影响新建立具有相同四元组的TCP连接。

   示例：当最后一个ACK丢失时，远程连接进入LAST-ACK状态，如果没有TIME-WAIT状态，主动调发起close的一方立即以同样的四元组建立新连接，当远程收到一个SYN 时，会回复一个RST包（Connection reset by peer），因为这SEQ不对，那么新的连接将无法建立成功，报错终止。
