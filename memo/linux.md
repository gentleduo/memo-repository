...
- up：机器运行了多长时间
- users：当前登录用户数
- load average：系统平均负载，包含3列，分别表示1分钟、5分钟和15分钟的系统平均负载。

如果将CPU比作是桥梁，对于单核的CPU就好比是单车道的桥梁。每次桥梁只能让一辆汽车通过，并且要以规定的速度通过。那么：

1. 如果每个时刻都只有一辆汽车通过，那么所有汽车都不用排队，此时桥梁的使用率最高。以平均负载 1.0 表示
2. 如果每个时刻都只有一辆汽车通过，那么所有汽车都不用排队，此时桥梁的使用率最高。以平均负载 1.0 表示
3. 如果每个时刻都只有一辆汽车通过，那么所有汽车都不用排队，此时桥梁的使用率最高。以平均负载 1.0 表示

系统的平均负载与上面的例子一样，在单核CPU的环境下：

1. 当平均负载等于1.0时，表示CPU使用率最高。
2. 当平均负载小于1.0时，表示CPU使用率处于空闲状态。
3. 当平均负载大于1.0时，表示CPU使用率已经超过负荷。

对于单核CPU来说，平均负载1.0表示使用率最高。但对于多核CPU来说，平均负载要乘以核心数。比如在4核CPU的系统中，当平均负载为4.0时，才表示CPU的使用率最高。
...
1. /etc/profile：用来设置系统环境参数，比如$PATH. 这里面的环境变量是对系统内所有用户生效的。但是只有用户登录时才会加载
...
>core文件：程序由于各种异常或者bug导致在运行过程中异常退出或者中止，并且在满足一定条件下会产生一个叫做core的文件。通常情况下，core文件会包含了程序运行时的内存，寄存器状态，堆栈指针，内存管理信息还有各种函数调用堆栈信息等，我们可以理解为是程序工作当前状态存储生成第一个文件，许多的程序出错的时候都会产生一个core文件，通过工具分析这个文件，可以定位到程序异常退出的时候对应的堆栈调用等信息，找出问题所在并进行及时解决。
>
>检查生成core文件的选项是否打开：ulimit -c
>
>对用户生成的core文件不做限制：ulimit -c unlimited
>
...
[root@server01 ~]# pidstat -w 1 3
Linux 3.10.0-693.el7.x86_64 (server01)  12/02/2024      _x86_64_        (1 CPU)

10:48:14 AM   UID       PID   cswch/s nvcswch/s  Command
10:48:15 AM     0         3      2.88      0.00  ksoftirqd/0
10:48:15 AM     0         9     19.23      0.00  rcu_sched
10:48:15 AM     0        20     11.54      0.00  kworker/0:1
10:48:15 AM     0       298      0.96      0.00  kworker/0:1H
10:48:15 AM     0       401     19.23      0.00  xfsaild/dm-0
10:48:15 AM     0      1297      0.96      0.96  pidstat

10:48:15 AM   UID       PID   cswch/s nvcswch/s  Command
10:48:16 AM     0         3      2.02      0.00  ksoftirqd/0
10:48:16 AM     0         9     19.19      0.00  rcu_sched
10:48:16 AM     0        20     16.16      0.00  kworker/0:1
10:48:16 AM     0        93      1.01      0.00  kworker/0:3
10:48:16 AM     0       298      1.01      0.00  kworker/0:1H
10:48:16 AM     0       401     20.20      0.00  xfsaild/dm-0
10:48:16 AM     0      1207      1.01      0.00  sshd
10:48:16 AM     0      1297      1.01      1.01  pidstat

```

如果对单个进程感兴趣，可以直接查看/proc/[pid]/status中的voluntary_ctxt_switches和nonvoluntary_ctxt_switches字段，例如：

```bash
[root@server01 ~]# cat /proc/1554/status 
Name:   python3
Umask:  0022
State:  D (disk sleep)
Tgid:   1554
...
Mems_allowed_list:      0
voluntary_ctxt_switches:        6407
nonvoluntary_ctxt_switches:     1231
```
...
```bash
# 判断上下文切换类型
[root@server01 ~]# grep 'voluntary_ctxt_switches' /proc/1554/status 
voluntary_ctxt_switches:        45603
nonvoluntary_ctxt_switches:     1256
[root@server01 ~]# grep 'nonvoluntary_ctxt_switches' /proc/1554/status 
nonvoluntary_ctxt_switches:     1256
```
...
# 使用 strace 分析系统调用：查看进程频繁调用的系统调用，是否存在大量 read、write、poll 或 futex 操作。
[root@server01 ~]# strace -p 1554
strace: Process 1554 attached
epoll_wait(3, 
#• 使用 iotop 检查 I/O 活动：
#• 使用 perf 检查锁竞争：
```
...
# 使用top或pidstat查看进程的CPU占用是否异常。
[root@server01 ~]# pidstat -u 1 -p 1554
Linux 3.10.0-693.el7.x86_64 (server01)  12/02/2024      _x86_64_        (1 CPU)

11:00:39 AM   UID       PID    %usr %system  %guest    %CPU   CPU  Command
11:00:40 AM     0      1554    0.00    0.00    0.00    0.00     0  python3
11:00:41 AM     0      1554    0.00    0.00    0.00    0.00     0  python3
11:00:42 AM     0      1554    0.00    0.00    0.00    0.00     0  python3
11:00:43 AM     0      1554    0.00    0.00    0.00    0.00     0  python3
#•观察系统负载：
#使用htop或uptime查看是否有高负载进程导致频繁抢占。
#•使用perf sched分析调度器行为：
```
