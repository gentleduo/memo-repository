...
graph LR;
   ...
   B[倒排表:<br>posting list]-->H[tip:词典索引,存放前缀后缀指针,需要内存加载<BR>tim:后缀词块,倒排表指针<BR>doc:倒排表,词频]
   C[词项字典term:<br>dictionary]-->H[tip:词典索引,存放前缀后缀指针,需要内存加载<BR>tim:后缀词块,倒排表指针<BR>doc:倒排表,词频]
   C[词项字典term:<br>dictionary]-->I[极大的节省内存,FST压缩倍率最高可以达到20倍,<BR>性能不如HashMap但也不错]
   D[词项索引<br>term index]-->I[极大的节省内存,FST压缩倍率最高可以达到20倍,<BR>性能不如HashMap但也不错]
